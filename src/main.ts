...
export async function run(): Promise<void> {
  try {
    ...
    if(fs.existsSync(`${workspacePath}/workspace/${tarball}`)) {
      ...
      await exec.exec(`git archive --output=${workspacePath}/home/rpmbuild/SOURCES/${tarball} --prefix=${name}-${version}/ HEAD`);
    ...
    let myOutput = '';
    await cp.exec(`ls ${workspacePath}/home/rpmbuild/SRPMS/`, (err, stdout, stderr) => {
      if (err) {
        //some err occurred
        core.error(err)
      } else {
    ...
    await exec.exec(`cp ${workspacePath}/home/rpmbuild/SRPMS/${myOutput} rpmbuild/SRPMS`);
